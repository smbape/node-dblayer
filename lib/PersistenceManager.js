...
(function() {
  var CompiledMapping, DeleteQuery, GenericUtil, InsertQuery, PRIMITIVE_TYPES, PersistenceManager, RowMap, SelectQuery, UpdateQuery, async, log4js, logger, semLib, squel, _, _addUpdateOrDeleteCondition, _addWhereCondition, _getInitializeCondition,
  ...
  module.exports = PersistenceManager = (function(_super) {
  ...
  PersistenceManager.prototype.save = function(model, options, callback) {
    var backup, className, definition, where;
  ...
  _getInitializeCondition = function(pMgr, model, className, definition, options) {
    var attr, attributes, connector, constraint, index, isSetted, prop, value, where, _i, _j, _k, _len, _len1, _len2, _ref;
    connector = options.connector;
    if (typeof options.where === 'undefined') {
      if (!model) {
        ...
        if (definition.constraints.unique.length !== 0 && (options.useAttributes === false || !options.attributes)) {
          attributes = {};
          _ref = definition.constraints.unique;
          for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
            constraint = _ref[index];
            isSetted = true;
            for (_j = 0, _len1 = constraint.length; _j < _len1; _j++) {
              prop = constraint[_j];
  ...
  _addWhereCondition = function(pMgr, model, attr, value, definition, connector, where, options) {
    var column, id, propClassName, propDef;
    if (typeof value === 'undefined' || !definition.availableProperties.hasOwnProperty(attr)) {
      return;
    }
  ...
  InsertQuery = (function() {
    function InsertQuery(pMgr, model, options) {
      var className, column, connector, definition, err, fields, handlers, id, idName, insert, insertHandler, mixin, parentModel, prop, propDef, value, writeHandler, _i, _len, _ref, _ref1;
      if (options == null) {
        options = {};
      }
      ...
      if (definition.mixins.length > 0) {
        ...
        this.toParam = function() {
          var index, mixin, params, _i, _len, _ref;
    ...
    InsertQuery.prototype.execute = function(connector, callback) {
      var idIndex, index, params, tasks, value, _i, _len, _ref;
      ...
      connector.query(query, function(err, res) {
        ...
        if (definition.id.hasOwnProperty('column')) {
          if (fields[definition.id.column]) {
            ...
            id = (res.rows instanceof Array) && res.rows.length > 0 && res.rows[0][definition.id.column];
          }
        }
  ...
  SelectQuery = (function() {
    ...
    SelectQuery.prototype.stream = function(streamConnector, listConnector, callback, done) {
      ...
      query = rowMap.parse(this.toString());
      pMgr = this.getManager();
      options = this.getOptions();
      ...
      return streamConnector.stream(query, (function(_this) {
        return function(row, stream) {
          var err, model, tasks;
          tasks = [];
          model = rowMap.initModel(row, null, tasks);
          if (tasks.length > 0) {
            if (listConnector === streamConnector || (listConnector.getInnerPool() === streamConnector.getInnerPool() && listConnector.getMaxConnection() < 2)) {
      ...
      })(this), ret, options.executeOptions);
    };

    SelectQuery.prototype.list = function(connector, callback) {
      var err, options, pMgr, query, rowMap;
      if ('function' !== typeof callback) {
      ...
      options = this.getOptions();
      connector.query(query, function(err, res) {
        var index, model, models, ret, row, rows, tasks, _i, _len;
        if (err) {
          return callback(err);
        }
        rows = res.rows;
  ...
  _addUpdateOrDeleteCondition = function(action, name, connector, pMgr, model, className, definition, options) {
    var condition, err, hasNoCondition, id, idName, where, _i, _len;
    idName = pMgr.getIdName(className);
  ...
  UpdateQuery = (function() {
    function UpdateQuery(pMgr, model, options) {
      var changeCondition, className, column, connector, constraint, definition, handlers, lock, lockCondition, parentModel, prop, propDef, result, update, updateHandler, value, writeHandler, _ref;
      if (options == null) {
        options = {};
      }
      this.getModel = function() {
      ...
      if (definition.mixins.length === 0) {
        ...
        this.toParam = function() {
          var index, mixin, params, _i, _len, _ref1;
          if (this.hasData) {
    ...
    UpdateQuery.prototype.execute = function(connector, callback) {
      var definition, hasUpdate, idIndex, index, params, tasks, value, _i, _ref;
      if (this.toString === this.oriToString) {
      ...
      idIndex = 0;
      tasks = [];
      for (index = _i = _ref = params.values.length - 1; _i >= 0; index = _i += -1) {
      ...
      async.series(tasks, (function(_this) {
        return function(err, results) {
          var extended, id, result, _j, _len;
          if (err) {
            return callback(err);
          }
          ...
          _this._execute(connector, function(err, id, extended) {
            ...
            callback(err, id, !hasUpdate && extended);
          });
        };
      })(this));
    };
  ...
  DeleteQuery = (function() {
    function DeleteQuery(pMgr, model, options) {
      var className, column, connector, definition, handlers, prop, propDef, remove, value, writeHandler, _ref;
      if (options == null) {
        options = {};
      }
      this.toParam = function() {
    ...
    DeleteQuery.prototype.execute = function(connector, callback) {
      var idIndex, index, params, tasks, value, _i, _ref;
      if (this.toString === this.oriToString) {
        return this._execute(connector, callback);
      }
      params = this.toParam();
      ...
      this._execute(connector, function(err) {
        if (err) {
          return callback(err);
        }
