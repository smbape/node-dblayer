...
(function() {
  var Connector, EventEmitter, GenericUtil, MAX_ACQUIRE_TIME, STATES, log4js, logger, semLib, _,
  ...
  module.exports = Connector = (function(_super) {
    ...
    Connector.prototype._begin = function(callback) {
      var err, query;
      if (this.savepoints === 1) {
        query = 'BEGIN';
      } else if (this.savepoints > 0) {
        query = 'SAVEPOINT sp_' + (this.savepoints - 1);
      } else {
    ...
    Connector.prototype._rollback = function(callback, all, errors) {
      ...
      if (this.savepoints === 1) {
        ...
        return callback(errors);
      } else if (this.savepoints === 2) {
        query = 'ROLLBACK';
      } else {
        query = 'ROLLBACK TO sp_' + (this.savepoints - 2);
      }
    ...
    Connector.prototype._commit = function(callback, all, errors) {
      ...
      if (this.savepoints === 1) {
        ...
        query = 'RELEASE SAVEPOINT sp_' + (this.savepoints - 2);
      }
      logger.trace('[query] - ' + query);
      return this._connection.query(query, (function(_this) {
        return function(err) {
