...
var GenericUtil, JOIN_FUNC, PlaceHolderParser, RowMap, STATIC, _, _coerce, _createModel, _createPlainObject, _getModelValue, _getPlainObjectValue, _handleRead, _readFields, _setModelValue, _setPlainObjectValue, fieldHolderParser, log4js, logger, path, placeHolderParser, squel,
  slice = [].slice;
...
module.exports = RowMap = (function() {
  ...
  RowMap.prototype._initRootElement = function(className, id, options) {
    ...
    if (options.hasOwnProperty('condition')) {
      ...
      select[hasJoin](this.escapeId(table), tableAlias, condition, type);
  ...
  RowMap.prototype._joinProp = function(id, parentInfo) {
    var column, connector, err, hasJoin, idColumn, info, parentDef, propDef, select, table, tableAlias;
    info = this._getInfo(id);
    ...
    if (parentInfo) {
      ...
      if (parentDef.properties.hasOwnProperty(info.attribute) && !parentDef.properties[info.attribute].nullable && parentInfo.hasJoin === JOIN_FUNC["default"]) {
        hasJoin = JOIN_FUNC["default"];
      }
    }
