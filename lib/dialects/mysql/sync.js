  ...
  getModel: function(connector, callback) {
    ...
    connector.query(query, function(err, result) {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        row = ref[i];
        ({TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME, DATA_TYPE, COLUMN_TYPE, EXTRA, NUMERIC_PRECISION, NUMERIC_SCALE, CHARACTER_MAXIMUM_LENGTH, DATE_TIME_PRECISION, IS_NULLABLE, COLUMN_DEFAULT, CONSTRAINT_NAME, CONSTRAINT_TYPE, CONSTRAINT_INDEX_NUM, REFERENCED_TABLE, REFERENCED_COLUMN, UPDATE_RULE, DELETE_RULE, INDEX_NAME, INDEX_NUM} = row);
