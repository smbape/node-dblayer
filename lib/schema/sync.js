...
_sync = function(oldModel, newModel, schema, options) {
  var alter, alters, create, drops, fkName, indexName, keys, lower_case_table_names, model, name, oldTableModel, opts, purge, queries, ref, ref1, tableName;
  queries = {
    creates: [],
    alters: [],
    drops: [],
  ...
  purge = options.purge, lower_case_table_names = options.lower_case_table_names;
...
_tableDiff = function(oldTableModel, newTableModel, options) {
  ...
  for (column in oldKeys) {
    ...
    if (newKeys && newKeys.hasOwnProperty(column)) {
      ...
      referenced_table = newKey.referenced_table;
